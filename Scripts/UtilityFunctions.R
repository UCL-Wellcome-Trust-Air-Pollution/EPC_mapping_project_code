...
# Define functions to prepare data and build logit model -----------------------

prepare_data_for_logit_model <- function(data,
                                         outcome_var,
                                         predictor_vars,
                                         numeric_to_factor_vars,
                                         most_recent_only){
  
  # If most_recent_only == TRUE, keep only most recent EPCs
  if(most_recent_only) data <- data %>% filter(most_recent == TRUE)
  
  # Set up data for model
  data_for_model <- data %>%
    
    # Select outcome var + predictor vars
    select(outcome_var, all_of(predictor_vars)) %>%
    
    # Filter missing variables
    filter(if_all(everything(), ~ !is.na(.))) %>%
    
    # Mutate specified numeric vars to factor
    mutate_at(vars(numeric_to_factor_vars), ~ as.factor(.))
...
build_logit_model <- function(data,
  ...
  # Build formula to pass to glm call
  formula <- as.formula(paste(outcome_var, "~", paste(predictor_vars, collapse = " + "), sep = ""))
...
# Function to download and clean Defra Black Carbon monitoring data for analysis

get_clean_defra_bc_data <- function(path_data_defra_bc,
                                    row_names,
                                    month_list,
                                    total_bc_infrared_col,
                                    total_bc_uv_col,
                                    low_wavelength,
                                    high_wavelength,
                                    angstrom_constant_ff,
                                    angstrom_constant_wb){
  
  # Download data from path
  data_defra_bc <- read.csv(here(path_data_defra_bc)) %>%
  ...
  # Extract metadata columns as lookup data to merge to hourly data
  metadata_lookup <- head(data_defra_bc, n = row_names - 1) %>%
    
    # Remove 'site_name' column
    select(!c(site_name, time)) %>%
    
    # Remove columns starting with 'x'
    select(!starts_with("x"))
  
  # Transpose metadata_lookup
  metadata_lookup <- as.data.frame(t(metadata_lookup)) %>%
    ...
    # Get rownames as variables
    tibble::rownames_to_column("site_name") %>%
    
    # Create ID column for later matching
    mutate(id = cur_group_id(), .by = "site_name")
  
  # Remove first four rows of metadata from main Defra data then pivot to long format
  data_defra_bc_long <- data_defra_bc %>%
    
    # Set rownames to column names
    row_to_names(row_number = row_names) %>%
    
    # Clean names again
    clean_names() %>%
    
    # Remove units columns
    select(!starts_with(c("status"))) %>%
    
    # Create date/time variable
    mutate(date = as.POSIXlt(paste(date, time), format = "%d/%m/%Y %H:%M:%S")) %>%
    
    # Remove 'time' column
    select(!time) %>%
    
    # Filter selected months
    filter(lubridate::month(date) %in% month_list) %>%
    
    # Mutate back to POSIXct format
    ...
    mutate(across(!date, ~ case_when(. == "No data" ~ NA,
                                   .default = .))) %>%
    
    # Mutate all as numeric
    mutate(across(!date, ~ as.numeric(.))) %>%
    
    # Rename first set of columns to have index 1
    rename_at(vars(black_carbon_880nm:uv_particulate_matter_370nm), ~ paste(., "_1", sep = "")) %>%
    
    # Pivot longer
    tidylog::pivot_longer(!date,
                          names_to = c(".value", "id"),
                          names_pattern = "^(.*)_([0-9]*)$") %>%
    
    # Change id var to numeric
    mutate(id = as.numeric(id)) %>%
    
    # Left join to metadata
    left_join(metadata_lookup, by = "id") %>%
    
    # Remove id column
    select(!id) %>%
    
    # Remove monitors in Scotland/Ireland
    filter(!str_detect(region, paste0(c("Ireland", "Scotland"), collapse = "|"))) %>%
    
    # Generate new column for hour of day and day of week
    mutate(hour = as.numeric(format(date, "%H")),
           day_of_week = weekdays(date)) %>%
    
    # Using below formula (details in manuscript appendix)
    mutate(wf_conc_uv = ({{total_bc_infrared_col}} - (low_wavelength/high_wavelength)^angstrom_constant_ff * {{total_bc_uv_col}}) / ((low_wavelength/high_wavelength)^angstrom_constant_wb - (low_wavelength/high_wavelength)^angstrom_constant_ff))
  
  # Return final data
  return(data_defra_bc_long)
